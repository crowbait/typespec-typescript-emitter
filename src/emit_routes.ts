import {EmitContext, emitFile, getDoc, Interface, isTemplateDeclaration, Namespace, navigateProgram, resolvePath} from '@typespec/compiler';
...
export const emitRoutes = async (
  ...
  const traverseNamespace = (n: Namespace | Interface, nestlevel: number, rootName: string) => {
    if (files[rootName] === undefined) files[rootName] = new AppendableString(
      `export const routes_${rootName} = {\n`
    );

    const numCollections = Array.from((n as any).interfaces ?? []).length + Array.from(((n as any).namespaces ?? []) as any[]).length;
    console.log(n.name, numCollections)

    let i = 1;
    for (const op of n.operations) {
      const httpOp = getHttpOperation(context.program, op[1])[0];

      const pathParams = httpOp.parameters.parameters.filter(p => p.type === "path");
      const urlParams = `params: {${pathParams
        .map(p => `${p.name}: string`)
        .join(", ")}}`;
      const urlReturn = pathParams
        .reduce(
          (sum, cur) => sum.replaceAll(`{${cur.name}}`, `${"$"}{params.${cur.name}}`) ,
          `\`${httpOp.path}\``
        );

      const doc = getDoc(context.program, op[1]);
      if (doc) files[rootName].addLine(`/** ${doc} */`.split("\n").map(l => `${"  ".repeat(nestlevel)}${l}`).join("\n"), 0);
      files[rootName]
        .addLine(`${op[1].name}: {`, nestlevel)
        .addLine(`verb: '${httpOp.verb.toUpperCase()}',`, nestlevel + 1)
        .addLine(`path: '${httpOp.path}',`, nestlevel + 1)
        .addLine(`getUrl: (${pathParams.length > 0 ? urlParams : ""}): string => ${urlReturn},`, nestlevel + 1);

      // auth
      let auth: Array<
        null | string | { apiKeyLocation: string; apiKeyName: string }
      > = [];
      const opAuth = getAuthentication(context.program, op[1]);
      if (opAuth) {
      ...
      files[rootName].addLine(i < n.operations.size || numCollections > 0 ? "}," : "}", nestlevel);
      i++;
    }

    let collectionNum = 1;
    const printCollection = (n: Namespace | Interface): void => {
      const doc = getDoc(context.program, n);
      if (doc) files[rootName].addLine(`/** ${doc} */`.split("\n").map(l => `${"  ".repeat(nestlevel)}${l}`).join("\n"));
      files[rootName].addLine(`${n.name}: {`, nestlevel);
      traverseNamespace(n, nestlevel + 1, rootName);
      files[rootName].addLine(collectionNum < numCollections ? "}," : "}", nestlevel);
      collectionNum++;
    }

    (((n as any).interfaces ?? []) as Interface[]).forEach(i => {if (!isTemplateDeclaration(i)) printCollection(i)});
    (((n as any).namespaces ?? []) as Namespace[]).forEach(ns => printCollection(ns));
    if (n.name === rootName && nestlevel === 1) files[rootName].addLine("} as const;");
  }
...
// emitting
  for (const file of Object.entries(files)) {
    await emitFile(context.program, {
      path: resolvePath(context.options['out-dir'], `routes_${file[0]}.ts`),
