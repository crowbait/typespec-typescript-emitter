...
export class ResolvableEnumMember extends Resolvable<EnumMember> {
  ...
  protected async type(opts: ResolverOptions<Resolver.Type>, out: ResolverResult<Resolver.Type>): Promise<void> {
    const foundParent = opts.typemap.find(mt => (
      mt.type.kind === "Enum" &&
      mt.type.name === this._t.enum.name &&
      compareArrays(mt.namespaces, namespaceListFromNamespace(this._t.enum.namespace) ?? [])
    ));
    if (foundParent) {
...
const resolveEnumMemberValue = (member: EnumMember, opts: ResolverOptions<any>): string => {
