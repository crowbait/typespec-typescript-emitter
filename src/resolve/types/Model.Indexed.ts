...
export class IndexedModel extends Resolvable<Model> {
  constructor(t: Model, r: Resolver) {
    ...
    } else throw new TypeError(`Cannot create IndexedModel on type of name ${t.name}`);
  }
  ...
  protected async typeguard(opts: ResolverOptions<Resolver.Typeguard>, out: ResolverResult<Resolver.Typeguard>): Promise<void> {
    ...
    switch (this._indexedModelKind) {
      case Type.Array:
        out.resolved.append(`Array.isArray(${opts.accessor}) && ${opts.accessor}.every((v) => ${resolved.resolved})`);
        break;
      case Type.Record:
        out.resolved.append(
  ...
  protected validate(): void {
    if (
      ...
      throw new Error("Tried creating IndexedModel for non-array and non-record model type");
    }
    if (!this._t.indexer) {
      this.diagnostic(
        "no-indexer",
