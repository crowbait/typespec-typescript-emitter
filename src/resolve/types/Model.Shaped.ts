...
export class ShapedModel extends Resolvable<Model> {
  ...
  private getPropVisibilityType(context: EmitContext<EmitterOptions>, prop: ModelProperty): VisibilityType[] | null {
    if (!prop.decorators.some(decorator => decorator.definition?.name === "@visibility")) return null;
    const ret: VisibilityType[] = [];
    if (isVisible(context.program, prop, Visibility.Read)) ret.push("Read");
    if (isVisible(context.program, prop, Visibility.Create)) ret.push("Create");
    if (isVisible(context.program, prop, Visibility.Update)) ret.push("Update");
    if (isVisible(context.program, prop, Visibility.Delete)) ret.push("Delete");
    if (isVisible(context.program, prop, Visibility.Query)) ret.push("Query");
    return ret;
  }
  ...
  public override async hasVisibility(opts: ResolverOptions<Resolver>, out: ResolverResult<Resolver>): Promise<boolean> {
    ...
    // resolve base
    if (this._t.baseModel && await Resolvable.hasVisibility(this._t.baseModel, opts, out, this._t)) return true;
    // resolve props: check for decorators
    const props = this.getProps();
    if (props.some(prop => prop.decorators.some(decorator => decorator.definition?.name === "@visibility"))) return true;
    ...
    for (const prop of props) {
      ...
      const propNamespaces = namespaceListFromNamespace((prop.type as any).namespace);
      if (opts.parents && opts.parents.some(parent => {
        const parentNamespaces = namespaceListFromNamespace((parent as any).namespace);
        return (
          parent.kind === prop.type.kind && 
          (prop.type as any).name &&
          (parent as any).name === (prop.type as any).name &&
          propNamespaces && parentNamespaces && compareArrays(propNamespaces, parentNamespaces)
        );
      })) continue;
      // not recursion, resolve
      if (await Resolvable.hasVisibility(prop.type, opts, out, this._t)) return true;
    }
    // nothing true
    return false;
  }
  ...
  protected async type(opts: ResolverOptions<Resolver.Type>, out: ResolverResult<Resolver.Type>): Promise<void> {
    ...
    if (this._t.baseModel) {
      const resolvedBase = await this.resolveNested(this._t.baseModel, opts, out)
    ...
    const propVisibilityTypes: Record<string, {type: VisibilityType[] | null, nested: string}> = {};
    ...
    if (props.length === 0) {
      ...
      for (let i = 0; i < props.length; i++) {
        ...
        const propVis = this.getPropVisibilityType(opts.context, prop);
      ...
      const propVisibilityTypesArr = Object.entries(propVisibilityTypes).filter(p => p[1].type !== null || p[1].nested);
      if (propVisibilityTypesArr.length > 0) {
        ...
        for (const [prop, propvis] of propVisibilityTypesArr) {
          ...
          if (propvis.type !== null && !propvis.nested) {
            visMap.addLine(`'${prop}': {${visLine}}`, opts.nestlevel + 1, "continued");
          }
          if (propvis.nested) {
            visMap.addLine(`'${prop}': {`, opts.nestlevel + 1)
            if (propvis.type) visMap.addLine(`${visLine},`, opts.nestlevel + 2);
            visMap.addLine(`nested: ${propvis.nested.split("\n").map(l => `  ${l}`).join("\n").trim()}`, opts.nestlevel + 2);
      ...
      if (
        opts.parents && opts.parents.every(p => (
          p.kind !== "Model" ||
          (p.name === "Array" || p.name === "Record")
        )) && visMap.value
      ) {
  ...
  protected async typeguard(opts: ResolverOptions<Resolver.Typeguard>, out: ResolverResult<Resolver.Typeguard>): Promise<void> {
    ...
    for (const prop of props) {
      ...
      const visibility = this.getPropVisibilityType(opts.context, prop);
