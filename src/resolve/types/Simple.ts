...
export class ResolvableSimple extends Resolvable<Type> {
  ...
  private static typeMap: Record<typeof allowedTypeKinds[number], (t: Type) => string> = {
  ...
  protected async type(opts: ResolverOptions<Resolver.Type>, out: ResolverResult<Resolver.Type>): Promise<void> {
    const mapped = ResolvableSimple.typeMap[this._t.kind as typeof allowedTypeKinds[number]]; // check is in validate()
  ...
  protected validate(): void {
    if (!ResolvableSimple.AllowedTypeKinds.includes(this._t.kind as any)) {
      ...
      throw new TypeError(`Type-kind/class mismatch: tried resolving type ${this._t.kind} as primitive`);
    }
  }
}