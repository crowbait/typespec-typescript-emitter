...
import { buildInterfaceMap, buildTypeMap } from "./helpers/buildTypeMap.js";
...
export async function $onEmit(context: EmitContext<EmitterOptions>) {
  ...
  const interfaceMap = buildInterfaceMap(context.program, context.options);

  if (context.options["enable-types"])
    await emitTypes(context.program, context.options, typeMap, interfaceMap);
